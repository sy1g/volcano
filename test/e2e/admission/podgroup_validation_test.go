...
var _ = ginkgo.Describe("PodGroup Validating Webhook E2E Test", func() {
	...
	ginkgo.It("Should reject PodGroup creation with closed queue", func() {
		...
		// First create a queue with Closed state
		queue := &schedulingv1beta1.Queue{
			ObjectMeta: metav1.ObjectMeta{
		...
		// Now try to create PodGroup that references this closed queue
		podGroup := &schedulingv1beta1.PodGroup{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: testCtx.Namespace,
	...
	ginkgo.It("Should reject PodGroup creation with non-existent queue", func() {
		...
		// Try to create PodGroup that references a non-existent queue
		podGroup := &schedulingv1beta1.PodGroup{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: testCtx.Namespace,
				Name:      "test-podgroup-non-existent-queue",
			},
			Spec: schedulingv1beta1.PodGroupSpec{
		...
		_, err := testCtx.Vcclient.SchedulingV1beta1().PodGroups(testCtx.Namespace).Create(context.TODO(), podGroup, metav1.CreateOptions{})
		gomega.Expect(err).To(gomega.HaveOccurred())
		gomega.Expect(err.Error()).To(gomega.ContainSubstring("unable to find queue"))
	})
	...
	ginkgo.It("Should reject PodGroup creation with closing queue", func() {
		...
		// First create a queue with Closing state
		queue := &schedulingv1beta1.Queue{
			ObjectMeta: metav1.ObjectMeta{
				Name: "test-queue-closing",
			},
			Spec: schedulingv1beta1.QueueSpec{
				Weight: 1,
			},
		...
		// Update queue status to Closing
		createdQueue.Status.State = schedulingv1beta1.QueueStateClosing
		_, err = testCtx.Vcclient.SchedulingV1beta1().Queues().UpdateStatus(context.TODO(), createdQueue, metav1.UpdateOptions{})
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

		// Now try to create PodGroup that references this closing queue
		podGroup := &schedulingv1beta1.PodGroup{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: testCtx.Namespace,
				Name:      "test-podgroup-closing-queue",
			},
			Spec: schedulingv1beta1.PodGroupSpec{
				Queue:             "test-queue-closing",
				MinMember:         1,
				PriorityClassName: "",
			},
		}
