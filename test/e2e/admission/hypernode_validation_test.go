...
var _ = ginkgo.Describe("HyperNode Validating Webhook E2E Test", func() {
	...
	// validate by crd  e.g. member selector must have one of exactMatch, regexMatch, or labelMatch
	ginkgo.XIt("Should reject HyperNode creation with multiple selector types (exactMatch and regexMatch)", func() {
		hyperNodeName := "invalid-multiple-selectors"
		testCtx := util.InitTestContext(util.Options{})
		defer util.CleanupTestContext(testCtx)
		...
		_, err := testCtx.Vcclient.TopologyV1alpha1().HyperNodes().Create(context.TODO(), hyperNode, metav1.CreateOptions{})
		gomega.Expect(err).To(gomega.HaveOccurred())
		gomega.Expect(err.Error()).To(gomega.ContainSubstring("cannot specify more than one selector type"))
	})

	// validate by crd  e.g. member selector must have one of exactMatch, regexMatch, or labelMatch
	ginkgo.XIt("Should reject HyperNode creation with multiple selector types (exactMatch and labelMatch)", func() {
		hyperNodeName := "invalid-multiple-selectors-exact-label"
		testCtx := util.InitTestContext(util.Options{})
		defer util.CleanupTestContext(testCtx)
	...
	// validate by crd  e.g. member selector must have one of exactMatch, regexMatch, or labelMatch
	ginkgo.XIt("Should reject HyperNode creation with no selector type", func() {
		hyperNodeName := "invalid-no-selector"
		testCtx := util.InitTestContext(util.Options{})
		defer util.CleanupTestContext(testCtx)
		...
		_, err := testCtx.Vcclient.TopologyV1alpha1().HyperNodes().Create(context.TODO(), hyperNode, metav1.CreateOptions{})
		gomega.Expect(err).To(gomega.HaveOccurred())
		gomega.Expect(err.Error()).To(gomega.ContainSubstring("member selector must have one of exactMatch, regexMatch, or labelMatch"))
	})
